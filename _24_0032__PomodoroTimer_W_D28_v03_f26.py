...
progress_50_min_timer = 50  # 3000 = 50 min, as default
short_10_min_timer = 10  # 600 = 10 min, as default
...
def reset_timer():
    ...
    canvas.itemconfig(timer_text, text="00:00", fill="white", font=(FONT_NAME, 30,
                                                                    "bold"))  # this code line needs to be placed before/above we .pack() it. # the implied x and y value are the *args; text="text_here" is the keyword arguments (**kwargs)   #timer_text is the variable that we link once we get the function 'count_down' fixed
...
def start_timer_button():  # this function is responsible for calling that function 'timer_count_down', and we drop the 'start_timer_button' part behind the command= of the Start button code block
    ...
    if reps == 4 or reps == 8 or reps == 12 or reps == 16 or reps == 20:
    ...
    elif reps == 2 or reps == 6 or reps == 10 or reps == 14 or reps == 18:
        count_down_function(short_break_countdown_seconds)
        timer_label.config(text="10 Minute Break", fg=PINK)
...
window.config(padx=100, pady=50,
              bg=YELLOW)  # this ACTUALLY ADDS to the original width and height of the original 421x518 window. It won't take AWAY, or keep it the same size, it ADDS to the x and y lengths. So (padx=40, pady=40) would be a total window size of 461x558 pixels.  # https://colorhunt.co/palette/ebe76cf0b86eed7b7b836096 for bg=background_color_of_choice (without using "", because )

fg = GREEN  # foreground

canvas = Canvas(width=430, height=496, bg=YELLOW,
                highlightthickness=0)  # variable canvas is created from the Canvas Widget  # width and height ar in pixel. # bg=background_color_for_canvas (not using "", since YELLOW is a variable name.  # highlightthickness=0 will get rid of the white line.
...
tomato_img = PhotoImage(
    file="tomato4.png")  # Also add the image=(PhotoImage datatype Class of tkinter), in order to read through a file and get ahold of a photo at a certain location.  (use Absolute File path, if in another dir). #Also will need to provide a variable name to store this image to (not the Class, but a different variable name of choice, like 'tomato_img'.), then place it after the 'image=' part of the next line:
canvas.create_image(215, 248,
                    image=tomato_img)  # add my image to it, x and y (which are implied, inside) is needed in the (). Because we want the image to be placed right in the center of the window, we divide the dimensions in half, for both, and place those numbers in the paren(). #Also add the image=(PhotoImage datatype Class of tkinter), in order to read through a file and get ahold of a photo at a certain location. Also add the variable after the 'image=' part.
timer_text = canvas.create_text(214, 305, text="00:00", fill="white", font=(FONT_NAME, 30,
                                                                            "bold"))  # this code line needs to be placed before/above we .pack() it. # the implied x and y value are the *args; text="text_here" is the keyword arguments (**kwargs)   #timer_text is the variable that we link once we get the function 'count_down' fixed
canvas.grid(column=1,
            row=1)  # to actually put it on the screen. Pack is okay for now, til we have to add more components. (changed to grid() )

#############

# Start Button:
button = Button(text="Start", font=20, highlightthickness=0,
                command=start_timer_button)  # command=miles_to_kilometers #ONCE the button is actually pressed, then you run the calculation from the miles and it rewrites over the km quantity converted amount and places it OVER the 0 that was written at the col=1, row=1 that was at the original spot for the km_result_label
button.grid(column=0, row=2)  # the fix was to reduce the col from 2 to 1, and row from 3 to 2.

#############

# Reset Button:
button = Button(text="Reset", font=20, highlightthickness=0,
                command=reset_timer)  # no () for the main_timer stop command  #ONCE the button is actually pressed, then you run the calculation from the miles and it rewrites over the km quantity converted amount and places it OVER the 0 that was written at the col=1, row=1 that was at the original spot for the km_result_label
button.grid(column=2, row=2)  # the fix was to reduce the col from 2 to 1, and row from 3 to 2.

#############

# checkmark label:
checkmark_label = Label(fg=GREEN, bg=YELLOW, font=("Arial", 24,
                                                   'bold'))  # color=GREEN   #label is a kwarg (see all key-value pairs on this line)   # was this before: text="This is old text",
checkmark_label.grid(column=1, row=3)
...
def count_down_function(count):
    ...
    if count > 0:
        ...
        main_timer = window.after(1000, count_down_function,
                                  count - 1)  # count_down should NOT be count_down() here.   #When/if we want to cancel the timer, we have to go back to the window.after( ) line, and we have to give it a name (put it inside a variable), then window.after_cancel() in the line above where we want to cancel it at.
    else:
        ...
        for _ in range(
                progress_sessions):  # warning, because reps divided by 2 or anything divided by anything, becomes a floating type number.
